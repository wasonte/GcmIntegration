...
public class GcmRegistrationIntentService extends IntentService {
    ...
    private void unregisterGCM(){
        ...
        try {
            ...
            // [END get_token]
            Log.i(TAG, "GCM Deleted Token: " + token);

            sendUnregistrationToServer(token);

            sharedPreferences.edit().putBoolean(GcmPreferences.DELETED_TOKEN_FROM_SERVER, true).apply();

        } catch (Exception e){
            Log.d(TAG, "Failed to complete token refresh", e);

            sharedPreferences.edit().putBoolean(GcmPreferences.DELETED_TOKEN_FROM_SERVER, false).apply();
        }

        // Notify UI that registration has completed, so the progress indicator can be hidden.
        Intent unregistrationComplete = new Intent(GcmPreferences.REGISTRATION_COMPLETE);
        unregistrationComplete.putExtra("register", false);
